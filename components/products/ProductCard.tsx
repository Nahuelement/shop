    ...
    return (
      ...
      >
          ...
          <CardActions sx={{ mt: '1vh', display: 'flex',flexDirection:'column',justifyContent:'space-evenly',alignItems:'flex-start'}} className='fadeIn'
              ...
              <Typography color='primary'  pb={0} pl={{xs:0,sm:2}} fontWeight={900}>{ product.title.split('%').slice(0,1) }</Typography>
              <Typography color='gray' sx={{whiteSpace: 'nowrap',width: '170px',overflow: 'hidden',textOverflow:'ellipsis'}} fontWeight={500}  pb={1} pl={{xs:0,sm:1}}>{ `${product.title.split('%').slice(1,-1)}`.replaceAll(",", " ")}</Typography>
              <Typography color='black' fontWeight={600} pl={{xs:0,sm:1}}>{ `$ ${product.price}` }</Typography>
          </CardActions>
