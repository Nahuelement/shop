    ...
    <ShopLayout title={ product.title } pageDescription={ product.description }>
      ...
      <Grid container spacing={6} pt={2}>
        ...
        <Grid item xs={ 12 } sm={ 6} sx={{marginTop:{xs:'-6vh',sm:'22vh'}}}>
          <Box display='flex'  flexDirection='column' paddingLeft={{xs:2,sm:0}} paddingRight={{xs:0,sm:2}}>

            {/* titulos */}
            <Typography variant='h1' color='primary' pb={1} component='h1'>{ `${product.title.split('%').slice(0,1)}` }</Typography>
            <Typography variant='h1' className='fontStyle' component='h1'>{ `${product.title.split('%').slice(1,-1)}`.replaceAll(",", " ") }</Typography>
